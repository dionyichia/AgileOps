  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto space-y-6 pb-4">
        ...
        {/* Recommendation card */}
        <div className="bg-gradient-to-br from-cerulean-900/60 to-[#111827] border border-cerulean-700/40 rounded-2xl p-6">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <div className="text-xs font-bold text-cerulean tracking-widest uppercase mb-1">Recommendation</div>
              <h2 className="text-xl font-bold text-white">
                Adopt <span className="text-cerulean-300">{toolName}</span> for your SDR team
              </h2>
            </div>
        ...
        <div className="bg-[#111827] border border-slate-800 rounded-2xl p-6">
          ...
          <div className="mt-4 mb-6">
            <div className="flex justify-between text-xs text-slate-400 mb-2">
              ...
              <span className="font-bold text-cerulean-300">{adoption}%</span>
            </div>
            <input
              type="range"
              min={10}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#0F1629] border border-slate-800 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                <Users size={15} className="text-cerulean" />
            ...
            <div className="bg-[#0F1629] border border-slate-800 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                <Building2 size={15} className="text-magenta-400" />
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {useCases.map((uc) => (
              <div key={uc.title} className="flex gap-3 bg-[#0F1629] border border-slate-800 rounded-xl p-4">
                <CheckCircle2 size={16} className="text-cerulean flex-shrink-0 mt-0.5" />
                <div>
        ...
        <div className="bg-[#111827] border border-slate-800 rounded-2xl p-6">
          ...
          <div className="text-sm text-slate-400 leading-relaxed mb-5">
            At <strong className="text-cerulean-300">{adoption}% adoption</strong>, integrating {toolName} into your SDR workflow
            is projected to save <strong className="text-sea-400">{timeSaved} hours/rep/week</strong> and increase qualified
            pipeline by <strong className="text-magenta-400">{throughput}%</strong> — generating an estimated{' '}
            <strong className="text-sea-400">{formatDollar(netRevenue)}</strong> in net annual value against a{' '}
          ...
          <div className="flex gap-3">
            <button
              onClick={() => {}}
              className="flex items-center gap-2 bg-cerulean hover:bg-cerulean-400 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors"
            >
              <Download size={15} />
              Download Report
            </button>
...
function Metric({ label, value, sub, color }: { label: string; value: string; sub: string; color: string }) {
  const colorMap: Record<string, string> = {
    emerald: 'text-sea-400',
    indigo:  'text-cerulean',
    violet:  'text-magenta-400',
    amber:   'text-gold-400',
...
function ConfidenceBadge({ score }: { score: number }) {
  const color = score >= 80 ? 'text-sea-400 bg-sea-500/10 border-sea-500/30'
              : score >= 60 ? 'text-gold-400 bg-gold-500/10 border-gold-500/30'
